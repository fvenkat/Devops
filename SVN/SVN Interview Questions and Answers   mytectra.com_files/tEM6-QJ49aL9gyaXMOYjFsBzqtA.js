;if(CloudflareApps.matchPage(CloudflareApps.installs['9FuzCcMUjbKg'].URLPatterns)){(function(){'use strict'
...
var options=window.CloudflareApps.installs['9FuzCcMUjbKg'].options
...
if("9FuzCcMUjbKg"==='preview')updateVisibility()}
function updateVisibility(){clearTimeout(visibilityTimeout)
var visible=true
var stored=false
try{stored=JSON.parse(store['cf-flashcard'])}catch(e){}
if("9FuzCcMUjbKg"==='preview'||!stored){visible=true}else{var timeframe=times[options.renew]
...
if("AdYSdzqiA6TN"==='preview'){desktopConfigs.responsive={maxWidth:600};mobileConfigs.responsive={maxWidth:600}}
return configs;};var setupAddthisTools=setInterval(function(){if(typeof window.addthis!=='undefined'){window.clearInterval(setupAddthisTools);if("AdYSdzqiA6TN"==='preview'){var parsed=CloudflareApps.proxy.originalURL.parsed;window.addthis_share={url:parsed.scheme+'://'+parsed.host+parsed.path,title:document.title}}
window.addthis.layers(getConfigurations(),layerCallback);}},200);var _postMessage=window.postMessage;if("AdYSdzqiA6TN"==='preview'){window.postMessage=function postMessage(){'[native code]'
...
window.CloudflareApps.installs['AdYSdzqiA6TN'].scope={setOptions(nextOptions){options=nextOptions;updateElement();}}}());};if(CloudflareApps.matchPage(CloudflareApps.installs['eXjG7tPV3Fhc'].URLPatterns)){(function(modules){var installedModules={};function __webpack_require__(moduleId){if(installedModules[moduleId]){return installedModules[moduleId].exports;}
...
var hash=void 0
var options=CloudflareApps.installs['eXjG7tPV3Fhc'].options
var product=CloudflareApps.installs['eXjG7tPV3Fhc'].product
...
if("eXjG7tPV3Fhc"==='preview'&&node.nodeName==='IFRAME'&&node.src.indexOf('https://embedded.cloudflareapps.com')!==-1){return}
...
if("eXjG7tPV3Fhc"==='preview'){messageIndex=previewMessageIndex}else{messageIndex=Math.floor(Math.random()*options.messages.length)}
...
if("eXjG7tPV3Fhc"!=='preview'&&(!shouldShow||hasSeenHash())){hideWelcomeBar()
return}
updateElementStyle()
element.innerHTML=''
...
window.CloudflareApps.installs['eXjG7tPV3Fhc'].scope={updateOptions:function updateOptions(nextOptions){if(nextOptions.messages.length!==options.messages.length){previewMessageIndex=nextOptions.messages.length-1}else{for(var i=0;i<nextOptions.messages.length;i++){var oldEntry=options.messages[i]
